...
| pg-boss-Worker für zeitgesteuerte Läufe | Die Runtime führt Läufe derzeit synchron über Server Actions aus. Für Zeitpläne und Retries im Hintergrund fehlt der Worker-Prozess. `pg-boss` ist als Abhängigkeit installiert, aber **nirgends verwendet** — das Verzeichnis `src/server/jobs/` existiert noch nicht. Der Feature-Flag „Zeitpläne" ist im Adminbereich entsprechend als wirkungslos gekennzeichnet | Deployment-Umgebung mit dauerhaftem Prozess | neu: `src/server/jobs/`, `src/server/agents/runtime/engine.ts` | Mittel |
| Periodenwechsel und automatische Rechnungsstellung | Verbrauchszähler laufen je Kalendermonat (`YYYY-MM`) und setzen sich damit selbst zurück; Rechnungen werden derzeit nur manuell über die Billing-Seite erzeugt. Für automatische Abrechnung fehlt derselbe Worker | pg-boss-Worker (siehe oben) | `src/server/billing/service.ts` (`issueInvoice`), neu: `src/server/jobs/` | Mittel |
